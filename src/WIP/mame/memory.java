...
import static old.arcadeflex.libc_old.fopen;
import static old.arcadeflex.libc_old.fprintf;
...
import static old.mame.driverH.CPU_16BIT_PORT;
import static old.mame.driverH.CPU_FLAGS_MASK;
import static old.mame.driverH.CPU_TMS34010;
import static old.mame.driverH.CPU_Z80;
...
/*TODO*///***************************************************************************/
    ...
    public static void cpu_setbankhandler_r(int bank, ReadHandlerPtr handler) {
        int offset = bankreadoffset[bank];
    ...
    public static void cpu_setbankhandler_r(int bank, int _handler) {
        int offset = 0;
        int u8_hardware;
        ReadHandlerPtr handler = null;
        switch (_handler) {
            case MRA_RAM:
            case MRA_ROM:
                handler = mrh_ram;
            ...
            case MRA_BANK15:
            case MRA_BANK16:
                u8_hardware = (int) MWA_BANK1 - (int) _handler + 1;
                handler = bank_read_handler[u8_hardware];
    ...
    public static void cpu_setbankhandler_w(int bank, WriteHandlerPtr handler) {
        int offset = bankwriteoffset[bank];
        memorywriteoffset[bank] = offset;
        memorywritehandler[bank] = handler;
    }

    /* set writememory handler for bank memory	*/
    public static void cpu_setbankhandler_w(int bank, int _handler) {
        int offset = 0;
        int u8_hardware;
        WriteHandlerPtr handler = null;
        switch (_handler) {
            case MWA_RAM:
                handler = mwh_ram;
                break;
            ...
            case MWA_BANK16:
                u8_hardware = (int) MWA_BANK1 - (int) _handler + 1;
                handler = bank_write_handler[u8_hardware];
