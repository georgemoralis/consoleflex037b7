...
public class amstrad
{
	...
	/*-------------------------------------------*/
	/* MULTIFACE */
	/*TODO*////static static WRITE_HANDLER(multiface_io_write);
	...
	*/
	...
	static int amstrad_cycles_at_frame_end = 0;
	/* cycle count of last write */
	static int amstrad_cycles_last_write = 0;
	static int time_delta_fraction = 0;
	...
	static int[] ppi_port_inputs=new int[3];
	static int[] ppi_port_outputs=new int[3];
	...
	static timer_entry amstrad_interrupt_timer;
	...
	static int AmstradCPC_GA_RomConfiguration;
	
	static int[] AmstradCPC_PenColours=new int[18];
	...
	/* ram configuration */
	static int AmstradCPC_GA_RamConfiguration;
	
	/* selected pen */
	static int AmstradCPC_GA_PenSelected;
	
	
	
	static int AmstradCPC_ReadKeyboard()
	{
	...
	public static void Amstrad_RethinkMemory()
	{
		 ...
		 * opcode and opcode argument reads */
		{
			...
			{
				BankBase = new UBytePtr(memory_region(REGION_CPU1), 0x010000);
			}
			else
			{
			...
			cpu_setbank(2, new UBytePtr(BankBase, 0x02000));
	
	
			/* bank 1 - 0x04000..0x07fff */
			cpu_setbank(3, AmstradCPC_RamBanks[1]);
			cpu_setbank(4, new UBytePtr(AmstradCPC_RamBanks[1], 0x02000));
	
			/* bank 2 - 0x08000..0x0bfff */
			cpu_setbank(5, AmstradCPC_RamBanks[2]);
			cpu_setbank(6, new UBytePtr(AmstradCPC_RamBanks[2], 0x02000));
			...
			cpu_setbank(8, new UBytePtr(BankBase, 0x02000));
	
			cpu_setbank(9, AmstradCPC_RamBanks[0]);
			cpu_setbank(10, new UBytePtr(AmstradCPC_RamBanks[0], 0x02000));
			cpu_setbank(11, AmstradCPC_RamBanks[1]);
			cpu_setbank(12, new UBytePtr(AmstradCPC_RamBanks[1], 0x02000));
			cpu_setbank(13, AmstradCPC_RamBanks[2]);
			cpu_setbank(14, new UBytePtr(AmstradCPC_RamBanks[2], 0x02000));
	...
	static void AmstradCPC_GA_SetRamConfiguration()
	{
		...
		BankAddr = new UBytePtr(Amstrad_Memory, (BankIndex << 14));
	...
	public static void AmstradCPC_GA_Write(int Data)
	{
	/*#ifdef AMSTRAD_DEBUG
		printf("GA Write: %02x\r\n", Data);
	#endif*/
		...
		case 1:
			{
				...
				/* colour selection */
				if ((AmstradCPC_GA_PenSelected & 0x010) != 0)
				{
				...
				AmstradCPC_PenColours[PenIndex] = (char)(Data & 0x01f);
	...
	public static ReadHandlerPtr AmstradCPC_ReadPortHandler = new ReadHandlerPtr() {
            public int handler(int offset) {
		int data = 0x0ff;
	...
	static UBytePtr multiface_ram;
	static int multiface_flags;
	
	/* stop button has been pressed */
	public static final int MULTIFACE_STOP_BUTTON_PRESSED	= 0x0001;
	/* ram/rom is paged into memory space */
	public static final int MULTIFACE_RAM_ROM_ENABLED	= 0x0002;
	/* when visible OUT commands are performed! */
	public static final int MULTIFACE_VISIBLE		= 0x0004;
	...
	public static opbase_handlerPtr amstrad_multiface_opbaseoverride=new opbase_handlerPtr() {            
	...
	public static int multiface_hardware_enabled()
	{
			if (multiface_ram!=null)
			{
	...
	public static void	multiface_stop()
	{
		/* multiface hardware enabled? */
			if (multiface_hardware_enabled()==0)
			return;
	...
	static void multiface_rethink_memory()
	{
			UBytePtr multiface_rom;
	
		/* multiface hardware enabled? */
			if (multiface_hardware_enabled()==0)
			return;
	
			multiface_rom = new UBytePtr(memory_region(REGION_CPU1), 0x01C000);
			...
			switch ((offset>>8) & 0x0ff)
			{
					...
					case 0x07f:
					{
							switch (data & 0x0c0)
							{
									...
									case 0x00:
									{
										multiface_ram.write(0x01fcf, data);
									...
									case 0x040:
									{
										...
										pen_index = multiface_ram.read(0x01fcf) & 0x0f;
	
										if ((multiface_ram.read(0x01fcf) & 0x010) != 0)
										{
	
											multiface_ram.write(0x01fdf + pen_index, data);
										}
										else
										{
											multiface_ram.write(0x01f90 + pen_index, data & 0x01f);
										}
									...
									case 0x080:
									{
										...
										multiface_ram.write(0x01fef, data);
									...
									case 0x0c0:
									{
										...
										multiface_ram.write(0x01fff, data);
					...
					case 0x0bc:
					{
							multiface_ram.write(0x01cff, data);
					}
					break;
					...
					case 0x0bd:
					{
							...
							reg_index = multiface_ram.read(0x01cff) & 0x0f;
	
							multiface_ram.write(0x01db0 + reg_index, data);
					}
					break;
					...
					case 0x0f7:
					{
					  multiface_ram.write(0x017ff, data);
					...
					case 0x0df:
					{
					   multiface_ram.write(0x01aac, data);
					}
					break;
	...
	*/
	...
	void amstrad_interrupt_timer_trigger_reset_by_vsync()
	{
		amstrad_52_divider_vsync_reset = 2;
	}
	
	public static void amstrad_interrupt_timer_update()
	{
	...
	//public static void	amstrad_interrupt_timer_callback(int dummy)
	public static timer_callback amstrad_interrupt_timer_callback = new timer_callback() {
	...
	static void amstrad_clear_top_bit_of_int_counter()
	{
	...
	static int[] amstrad_cycle_table_op=
	{
	...
	static int[] amstrad_cycle_table_cb=
	{
			/* RLC */
	...
	static int[] amstrad_cycle_table_ed=
	{
			/* 0x00-0x03f */
			US_TO_CPU_CYCLES(2), US_TO_CPU_CYCLES(2), US_TO_CPU_CYCLES(2), US_TO_CPU_CYCLES(2),
			US_TO_CPU_CYCLES(2), US_TO_CPU_CYCLES(2), US_TO_CPU_CYCLES(2), US_TO_CPU_CYCLES(2),
	...
	static int[] amstrad_cycle_table_xy=
	{
	...
	static int[] amstrad_cycle_table_xycb=
	{
			/* 0x00-0x03f */
	...
	static int[] amstrad_cycle_table_ex=
	{
	...
	public static void Amstrad_Reset()
	{
	...
	//public static void amstrad_init_palette(UBytePtr sys_palette, UBytePtr sys_colortable, UBytePtr color_prom)
	 public static VhConvertColorPromPtr amstrad_init_palette = new VhConvertColorPromPtr() {
	...
	static MemoryReadAddress readmem_amstrad[] =
	{
		...
		new MemoryReadAddress(0x06000, 0x07fff, MRA_BANK4),
		new MemoryReadAddress(0x08000, 0x09fff, MRA_BANK5),
		new MemoryReadAddress(0x0a000, 0x0bfff, MRA_BANK6),
		new MemoryReadAddress(0x0c000, 0x0dfff, MRA_BANK7),
		new MemoryReadAddress(0x0e000, 0x0ffff, MRA_BANK8),
		new MemoryReadAddress(0x010000, 0x013fff, MRA_ROM),	   /* OS */
		new MemoryReadAddress(0x014000, 0x017fff, MRA_ROM),	   /* BASIC */
		new MemoryReadAddress(0x018000, 0x01bfff, MRA_ROM),	   /* AMSDOS */
		new MemoryReadAddress(-1)							   /* end of table */
	};
	
	
	static MemoryWriteAddress writemem_amstrad[] =
	{
		new MemoryWriteAddress(0x00000, 0x01fff, MWA_BANK9),
		new MemoryWriteAddress(0x02000, 0x03fff, MWA_BANK10),
		new MemoryWriteAddress(0x04000, 0x05fff, MWA_BANK11),
		new MemoryWriteAddress(0x06000, 0x07fff, MWA_BANK12),
		new MemoryWriteAddress(0x08000, 0x09fff, MWA_BANK13),
		new MemoryWriteAddress(0x0a000, 0x0bfff, MWA_BANK14),
		new MemoryWriteAddress(0x0c000, 0x0dfff, MWA_BANK15),
		new MemoryWriteAddress(0x0e000, 0x0ffff, MWA_BANK16),
	...
	static IOReadPort readport_amstrad[] =
	{
		new IOReadPort(0x0000, 0x0ffff, AmstradCPC_ReadPortHandler),
		new IOReadPort(-1)							   /* end of table */
	};
	
	static IOWritePort writeport_amstrad[] =
	{
		new IOWritePort(0x0000, 0x0ffff, AmstradCPC_WritePortHandler),
	...
	//#define KEYBOARD_PORTS \
        static InputPortPtr KEYBOARD_PORTS = new InputPortPtr(){ public void handler() {
		/* keyboard row 0 */ 
		PORT_START();  
	...
	//#define MULTIFACE_PORTS \
        static InputPortPtr MULTIFACE_PORTS = new InputPortPtr(){ public void handler() {
		PORT_START();  
	...
	static InputPortPtr input_ports_amstrad = new InputPortPtr(){ public void handler() { 
	
		KEYBOARD_PORTS.handler();
		...
		MULTIFACE_PORTS.handler();
	
	INPUT_PORTS_END(); }}; 
	
	static InputPortPtr input_ports_kccomp = new InputPortPtr(){ public void handler() { 
			KEYBOARD_PORTS.handler();
	INPUT_PORTS_END(); }}; 
	
	static Wave_interface wave_interface = new Wave_interface(
		1,		/* 1 cassette recorder */
		new int[]{ 50 }	/* mixing levels in percent */
        );
        
        //static void amstrad_shutdown_machine()
	...
	static MachineDriver machine_driver_amstrad = new MachineDriver
	(
		...
		new MachineCPU[] {
			...
			new MachineCPU(
				...
				null, 0	/* every scanline */
			),
		},
		new Float(50.08),							   /* frames per second */
	...
	(
		...
		new MachineCPU[] {
			...
			new MachineCPU(
				...
				readport_amstrad,		   /* IOReadPort */
				writeport_amstrad,		   /* IOWritePort */
				null,						   /*amstrad_frame_interrupt, *//* VBlank * Interrupt */
                                0,
                                null, 0
			)
		},
