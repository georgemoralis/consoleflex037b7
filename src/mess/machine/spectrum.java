...
import static old.mame.cpuintrfH.cpu_geturnpc;
...
public class spectrum {
    ...
    public static io_initPtr spectrum_rom_load = new io_initPtr() {
        public int handler(int id) {
            ...
            /*TODO*///file = image_fopen(IO_SNAPSHOT, id, OSD_FILETYPE_IMAGE_RW, OSD_FOPEN_READ);
            file = image_fopen(IO_SNAPSHOT, id, OSD_FILETYPE_IMAGE_R, 0);//to be replaced!!
     ...
     * *****************************************************************
     *
     * Override load routine (0x0556 in 48K ROM) if loading .TAP files Tape
     * blocks are as follows. 2 bytes length of block excluding these 2 bytes
     * (LSB first) 1 byte flag byte (0x00 for headers, 0xff for data) n bytes
     * data 1 byte checksum
     *
     * The load routine uses the following registers: IX Start address for block
     * DE Length of block A Flag byte (as above) Carry Flag Set for Load, reset
     * for verify
     *
     * On exit the carry flag is set if loading/verifying was successful.
     *
     * Note: it is not always possible to trap the exact entry to the load
     * routine so things get rather messy!
     *
     ******************************************************************
     */
    public static opbase_handlerPtr spectrum_tape_opbaseoverride = new opbase_handlerPtr() {
        public int handler(int address) {
            System.out.println("TODO spectrum_tape_opbaseoverride");
            return 0;//Dummy doesn't work
 ...
 /*TODO*/////	do
     ...
     *
     * if port_7ffd_data is -1 then machine is 48K - no paging if port_1ffd_data
     * is -1 then machine is 128K if neither port is -1 then machine is +2a/+3
     *
     * Note: the 128K .SNA and .Z80 file formats do not store the port 1FFD
     * setting so it is necessary to calculate the appropriate value for the ROM
     * paging.
     *
     ******************************************************************
     */
    static void spectrum_update_paging() {
        ...
        if (spectrum_plus3_port_1ffd_data == -1) {
            ...
            if ((spectrum_128_port_7ffd_data & 0x10) != 0) /* Page in Spec 48K basic ROM */ {
     ...
     *
     * 48K Format as follows: Offset Size Description (all registers stored with
     * LSB first) 0 1 I 1 18 HL',DE',BC',AF',HL,DE,BC,IY,IX 19 1 Interrupt (bit
     * 2 contains IFF2 1=EI/0=DI 20 1 R 21 4 AF,SP 25 1 Interrupt Mode
     * (0=IM0/1=IM1/2=IM2) 26 1 Border Colour (0..7) 27 48K RAM dump
     * 0x4000-0xffff PC is stored on stack.
     *
     * 128K Format as follows: Offset Size Description 0 27 Header as 48K 27 16K
     * RAM bank 5 (0x4000-0x7fff) 16411 16K RAM bank 2 (0x8000-0xbfff) 32795 16K
     * RAM bank n (0xc000-0xffff - currently paged bank) 49179 2 PC 49181 1 port
     * 7FFD setting 49182 1 TR-DOS rom paged (1=yes) 49183 16K remaining RAM
     * banks in ascending order
     *
     * The bank in 0xc000 is always included even if it is page 2 or 5 in which
     * case it is included twice.
     *
     ******************************************************************
     */
    public static void spectrum_setup_sna(UBytePtr pSnapshot, long SnapshotSize) {
        ...
        if (SnapshotDataSize == 49179) /* 48K Snapshot */ {
        ...
        if (SnapshotDataSize == 49179) {
            ...
            usedbanks[2] = 1;
            /* 0x8000-0xbfff */
            usedbanks[spectrum_128_port_7ffd_data & 0x07] = 1;
            /* Banked memory */
 ...
 /* repetition */
    ...
    public static void spectrum_setup_z80(UBytePtr pSnapshot, long SnapshotSize) {
        ...
        /*TODO is it neccesary??*/ pSnapshotData.offset = 0;
 ...
 /* SP */
        ...
        if ((pSnapshot.read(6) != 0) | (pSnapshot.read(7) != 0)) {
            ...
            if (is48ksnap != 0) /* Ensure 48K Basic ROM is used */ {
                spectrum_page_basicrom();
            }
                ...
                if ((pSource.memory.length - pSnapshot.memory.length) < SnapshotDataSize) {
                    ...
                    if (is48ksnap != 0) {
                        ...
                        if ((page >= 3) && (page <= 10)) {
                            ...
                            spectrum_update_paging();
                            Dest = 0x0c000;
                        } else /* Other values correspond to ROM pages */ {
                            Dest = 0x0;
                        }
                    }
     ...
     * *****************************************************************
     *
     * Returns 1 if the specified z80 snapshot is a 48K snapshot and 0 if it is
     * a Spectrum 128K or SamRam snapshot.
     *
     ******************************************************************
     */
 ...
 /*TODO*/////		if (device_open(IO_CASSETTE, id, 1, &wa))
    ...
    public static io_openPtr spec_quick_open = new io_openPtr() {
        public int handler(int id, int mode, Object arg) {
            ...
            /*TODO is that neccesary?*/ quick.data.offset = 0;
