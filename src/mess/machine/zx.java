...
public class zx {
	...
	public static UBytePtr tape_image = null;
	...
	public static timer_callback tape_bit_shift = new timer_callback() {
		...
		if (tape_header > 0)
		{
			...
			tape_header--;
			zx_frame_time = 15;
			/*TODO*////sprintf(zx_frame_message, "Tape header %5d", tape_header);
			return;
		}
		...
		if (tape_trailer > 0)
		{
			...
			tape_trailer--;
			zx_frame_time = 15;
			/*TODO*////sprintf(zx_frame_message, "Tape trailer %5d", tape_trailer);
			return;
		}
		...
		if (tape_bits == 0)
		{
			if (tape_name_offs < tape_name_size)
			{
				...
				tape_name_offs++;
				zx_frame_time = 15;
				/*TODO*////sprintf(zx_frame_message, "Tape name %04X:%02X", tape_name_offs, tape_data);
			}
			/*TODO*////else if (osd_fread(tape_file, tape_data, 1) == 1)
			/*TODO*////{
			/*TODO*////	logerror("TAPE data @$%04X: $%02X (%02X AF:%04X BC:%04X DE:%04X HL:%04X)\n", tape_data_offs, tape_data,
			/*TODO*////				tape_mask, cpu_get_reg(Z80_AF), cpu_get_reg(Z80_BC), cpu_get_reg(Z80_DE), cpu_get_reg(Z80_HL));
			/*TODO*////	tape_bits = 8;
			/*TODO*////	tape_data_offs++;
			/*TODO*////	zx_frame_time = 15;
				/*TODO*////sprintf(zx_frame_message, "Tape data %04X:%02X", tape_data_offs, tape_data);
			/*TODO*////}
	...
	static void extract_name()
	{
		...
		UBytePtr ram = memory_region(REGION_CPU1), name;
	
		tape_name[0] = '\0';
		/* find a LOAD token starting at (DE) */
		/*TODO*/////name = memchr(ram.read(de), 0xef, 32);
		name = new UBytePtr();
		//if ((name && name.read(1)) == 0x0b)	   /* load followed by doublequote? */
		if ((name.read(1)) == 0x0b)	   /* load followed by doublequote? */
		{
			//name += 2;
		...
		else
		{
			...
			if (cycles_this_bit - cycles_last_bit < 64)
			{
				...
				if (fast_read_count > 64)
				{
					...
					if (tape_name[0] != 0)
					{
						/*TODO*////char[] ext = tape_name + strlen(tape_name);
						char[] ext = {'.','P'};
	
						strcpy(ext, ".P");
						tape_file = osd_fopen(Machine.gamedrv.name, String.valueOf(tape_name), OSD_FILETYPE_ROM, 0);
						if (tape_file != null)
						{
							strcpy(ext, ".81");
							tape_file = osd_fopen(Machine.gamedrv.name, String.valueOf(tape_name), OSD_FILETYPE_ROM, 0);
						...
						{
							strcpy(ext, ".P");
							tape_file = osd_fopen(Machine.gamedrv.clone_of.name, String.valueOf(tape_name), OSD_FILETYPE_ROM, 0);
						}
						if ((tape_file!=null) && (Machine.gamedrv.clone_of != null))
						{
							strcpy(ext, ".81");
